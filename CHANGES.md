...
- `Bi_mappable`: `Fix_left` and `Fix_right`'s signatures no longer destructively
  substitute `t`, so  `T_alist.Fix_left(String).t` should now work.  This
  _shouldn't_ break existing code, as `Fix_left` and `Fix_right` are intended
  as extensions generated by Travesty.
